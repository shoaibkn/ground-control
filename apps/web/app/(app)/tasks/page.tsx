...
export default function TasksPage() {
  ...
  const getPriorityStyle = (priority: string) => {
    switch (priority) {
      ...
      case "Critical":
        return "bg-red-50 text-red-700 border-red-200/30 dark:bg-red-950/50 dark:text-red-300 dark:border-red-800/30 font-semibold animate-pulse"
      default:
  ...
  return (
    <div className="flex flex-row items-start w-full">
      <TasksSidebar />
      <div className="mt-2 mr-2 ml-4 flex h-full w-full flex-col min-w-0">
                ...
                <TableBody>
                  {filteredTasks && filteredTasks.length > 0 ? (
                    filteredTasks.map((task: any) => (
                      <TableRow key={task._id} className="hover:bg-muted/15 transition-colors">
                        {/* Title & Desc */}
                        <TableCell>
