...
function AcceptInvitationContent() {
    ...
    const fetchInvite = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setInviteError(
            error.message ||
              "Failed to load invitation details. The invite may have expired or been canceled."
          )
        } else {
          setInvite(data)
        }
        ...
        console.error(err)
        setInviteError(
          "An unexpected error occurred while loading the invitation."
        )
      } finally {
        setLoadingInvite(false)
      }
  ...
  const handleRedirectToLogin = () => {
    router.push(
      `/sign-in?redirectTo=${encodeURIComponent(window.location.href)}`
    )
  }

  const handleRedirectToSignUp = () => {
    router.push(
      `/sign-up?redirectTo=${encodeURIComponent(window.location.href)}`
    )
  }
  ...
  if (inviteError) {
    return (
      <Card className="w-full max-w-md border-destructive/20 bg-background/50 backdrop-blur-md">
        ...
        <CardContent className="p-6 pt-0 text-center text-sm font-medium text-muted-foreground">
          {inviteError}
        </CardContent>
        <CardFooter className="flex justify-center border-t border-border/20 p-6">
  ...
  if (!session) {
    return (
      <Card className="w-full max-w-md border-muted bg-background/50 backdrop-blur-md">
        <CardHeader className="text-center">
          <CardTitle className="text-xl font-bold">Join Organization</CardTitle>
          <CardDescription>
            You must be signed in to accept this invitation.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 pt-0 text-center text-sm leading-relaxed text-muted-foreground">
          Please sign in or create an account to accept the invitation. If you
          do not have an account yet, you can create one in seconds.
        </CardContent>
        <CardFooter className="flex flex-col gap-2 border-t border-border/20 p-6">
    ...
    <Card className="w-full max-w-md border-muted bg-background/50 backdrop-blur-md">
      ...
      <CardContent className="space-y-4 p-6 pt-0 text-center">
        <div className="space-y-2 rounded-lg border border-border/40 bg-muted/30 p-4">
          <p className="text-sm text-muted-foreground">Invited to join:</p>
